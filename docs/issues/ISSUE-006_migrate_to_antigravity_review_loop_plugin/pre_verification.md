...
### 1.2. UX・エッジケース
- **新規開発者の環境セットアップ**:
  - `git clone` 直後は submodule が空ディレクトリとなる可能性があるため、`agentSkillChecker.js` で `git submodule update --init --recursive` の案内を自動表示し、ビルドや品質ゲートで明瞭に通知する。
- **Inner Loop 高速反復**:
...
### 1.4. テスト自律性
- **品質ゲートの自律検証**:
  - `npm.cmd run check` により、シークレットスキャン、プラグイン整合性検証、ADR整合性、Issue整合性、TypeScript型検査、単体テスト、本番ビルドをワンショットで自律実行可能。
...
- **チェッカー (`scripts/checkers/agentSkillChecker.js`)**:
  - 旧ファイルを検査していたコードを、プラグイン Submodule 整合性検査にリファクタリング。パッチワークではなく責務を正しく更新。
