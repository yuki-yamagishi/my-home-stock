...
- **ステータス**: ✅ `status: closed` (ISSUE-007 / ADR-0010 にて実現・昇華)
- **種別**: 🟢 `type: feature`
- **担当者**: AIエージェント
- **作成日**: 2026-09-06
- **関連 ADR**: ADR-0001, ADR-0002, ADR-0010 (ISSUE-007)
...
- [x] 世帯 ID を変更すると、その世帯の在庫アイテムのみが表示されること（ISSUE-007 にて所属世帯ごとの完全分離として実現）。
- [x] 家族の端末で同一の世帯 ID を指定した場合、更新内容が共有されること（ISSUE-007 にて家族メール招待制による同一世帯共有として実現）。
- [x] `npm run check` が全件合格すること。
