...
3. **無検証直接プッシュの禁止と Pull Request 自動起票**:
   ...
   - GitHub Actions の再帰実行防止仕様により、デフォルト `GITHUB_TOKEN` で作成された PR では `pull_request` イベント（`ci.yml`）が自動発火しない。そのため、ワークフロー内の事前検証ステップ（下記 4）を強化して PR 作成前にテストを完全網羅し、人間開発者が最終確認・マージを行う運用とする（憲章 2.3 人間マージ専権の徹底）。
4. **ワークフロー内での品質事前検証の強化 (多重防衛)**:
   - PR 作成前に、Node.js 22 環境で `qualityGateRunner.js`（プラグイン展開ガード、ドキュメント整合性ガード、シークレット漏洩スキャン）に加えて、フロントエンドの厳格型検査（`npm run type-check`）および単体テスト全件（`npm run test:run`）を実行し、サブモジュール更新によって親リポジトリが破壊されていないかを 100% 事前保証する。
