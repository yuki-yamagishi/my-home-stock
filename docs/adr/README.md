...
| **[ADR-0013](0013-automated-submodule-update-workflow.md)** | repository_dispatch と PR 自動起票による Git Submodule 自動同期アーキテクチャ | 承認済 | 2026-09-18 |
...
3. **自動検証**: `npm run check:docs`（プラグインの `qualityGateRunner.js`）により、新規 ADR ファイルが本テーブルに登録されているか機械的に検証されます。
